...
public class BSPBuilder {
	...
	private GeneratingActivity genAct;// generating activity
	...
	public BSPBuilder(GeneratingActivity genAct, Distance dists, Cells cells, int width, int height, int colchange, int expectedPartiters) {
		super();
		this.genAct = genAct;
		this.dists = dists;
		this.cells = cells;
	...
	*/
	private void updateProgressBar(int partiters) {
		if (genAct.increasePercentage(partiters * 100 / expectedPartiters)) {
			try {
				Thread.currentThread().sleep(10);
			} catch (Exception e) { }
		}
