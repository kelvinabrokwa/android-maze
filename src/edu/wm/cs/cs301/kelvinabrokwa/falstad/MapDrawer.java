...
import edu.wm.cs.cs301.kelvinabrokwa.falstad.GraphicsWrapper.*;
...
public class MapDrawer extends DefaultViewer {
	...
	@Override
	public void redraw(GraphicsWrapper panel,
					   int state,
					   int px,
					   int py,
					   int view_dx,
		...
		if (state != Constants.STATE_PLAY)
			return ;
		if (maze.isInMapMode()) {
			draw_map(panel, px, py, walk_step, view_dx, view_dy, maze.isInShowMazeMode(), maze.isInShowSolutionMode()) ;
			draw_currentlocation(panel, view_dx, view_dy) ;
		}
	}
	// redraw method for android (no state parameter)
	public void redraw(GraphicsWrapper panel,
					   int px,
					   int py,
					   int view_dx,
	 ...
	 */
	public void draw_map(GraphicsWrapper panel, int px, int py, int walk_step, int view_dx, int view_dy, boolean showMaze, boolean showSolution) {
		panel.setColor(GWColor.white);
		...
		for (int y = ymin; y <= ymax; y++)
			for (int x = xmin; x <= xmax; x++) {
				...
				panel.setColor(seencells.hasWallOnTop(x, y) ? GWColor.white : GWColor.gray);
				...
				panel.setColor(seencells.hasWallOnLeft(x, y) ? GWColor.white : GWColor.gray);
	 ...
	 * @param gc
	 */
	public void draw_currentlocation(GraphicsWrapper panel, int view_dx, int view_dy) {
		panel.setColor(GWColor.red);
	 ...
	 * @param offy
	 */
	public void draw_solution(GraphicsWrapper panel, int offx, int offy, int px, int py) {
		// check parameters:
		...
		while (d > 1) {
			...
			// do the graphics
			panel.drawLine(nx1, ny1, nx1+ndx, ny1+ndy);
