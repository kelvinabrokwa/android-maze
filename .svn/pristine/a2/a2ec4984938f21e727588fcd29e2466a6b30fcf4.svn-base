...
import edu.wm.cs.cs301.kelvinabrokwa.ui.GeneratingActivity;
...
public class MazeBuilder implements Runnable {
	...
	protected void generate() {
		...
		final int[] remote = Globals.dists.computeDistances(Globals.cells) ;

		// identify cell with the greatest distance
		final int[] pos = Globals.dists.getStartPosition();
		//startx = pos[0] ;
		//starty = pos[1] ;
		Globals.startx = pos[0] ;
		Globals.starty = pos[1] ;

		// make exit position at true exit in the cells data structure
		Globals.cells.setExitPosition(remote[0], remote[1]);
	}
	...
	protected void generatePathways() {
		...
		int dir = 0; 	 	
		int origdir = dir; 	
		Globals.cells.setCellAsVisited(x, y); 
		while (true) { 		
			int dx = Constants.DIRS_X[dir];
			int dy = Constants.DIRS_Y[dir];
			if (!Globals.cells.canGo(x, y, dx, dy)) { 
				dir = (dir+1) & 3; 
				if (origdir == dir) { 
				...
				Globals.cells.deleteWall(x, y, dx, dy);
				x += dx;
				y += dy;
				Globals.cells.setCellAsVisited(x, y);
				origdirs[x][y] = dir;
	...
	private boolean placeRoom() {
		...
		if (Globals.cells.areaOverlapsWithRoom(rx, ry, rxl, ryl))
			return false ;
		...
		Globals.cells.markAreaAsRoom(rw, rh, rx, ry, rxl, ryl); 
		return true;
	}
	...
	*/
	public void build(GeneratingActivity act, int w, int h, int roomct, int pc) {
		init(act, w, h, roomct, pc);
		buildThread = new Thread(this);
		buildThread.start();
	}
	...
	private void init(Maze mz, int w, int h, int roomct, int pc) {
		...
		Globals.cells = new Cells(w,h) ;
		Globals.dists = new Distance(w,h) ;
		//colchange = random.nextIntWithinInterval(0, 255); // used in the constructor for Segments  class Seg
	}
	*/
	// android version
	private void init(GeneratingActivity ga, int w, int h, int roomct, int pc) {
		// store parameters
		//maze = mz;
		genAct = ga;
		width = w;
	...
	public void run() {
		...
		try {
			...
			final BSPBuilder b = new BSPBuilder(genAct, Globals.dists, Globals.cells, width, height, colchange, expectedPartiters) ;
			...
			genAct.newMaze(root, Globals.cells, Globals.dists, Globals.startx, Globals.starty);
		}
