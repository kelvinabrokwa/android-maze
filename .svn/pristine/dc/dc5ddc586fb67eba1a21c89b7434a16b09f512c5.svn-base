...
public class MazeBuilderPrim extends MazeBuilder implements Runnable {
	...
	protected void generatePathways() {
		...
		while(!candidates.isEmpty()){
			...
			if (Globals.cells.canGo(curWall.x, curWall.y, curWall.dx, curWall.dy))
			{
				// delete wall from maze, note that this takes place from both directions
				Globals.cells.deleteWall(curWall.x, curWall.y, curWall.dx, curWall.dy);
				// update current position
				x = curWall.x + curWall.dx;
				y = curWall.y + curWall.dy;
				
				Globals.cells.setCellAsVisited(x, y); // the flag is never reset, so this ensure we never go to (x,y) again
	...
	private void updateListOfWalls(int x, int y, ArrayList<Wall> walls) {
		if (Globals.cells.canGo(x, y, 0, 1))
		{
		...
		if (Globals.cells.canGo(x, y, 1, 0))
		{
